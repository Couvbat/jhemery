...
<template>
  <!--
    Teleported to <body>: the modal is declared inside NavBar's <header>, which
    carries `backdrop-blur`. A backdrop-filter makes an element the containing
    block for its fixed-position descendants, so without this the `fixed inset-0`
    overlay would size itself to the 56px header strip and push the close button
    off-screen.
  -->
  <Teleport to="body">
    <Transition
